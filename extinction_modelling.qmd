...
author-title: Leading the practical
...
Think of projects and a folder where you would keep all your input data, output figures, output summary data etc.

Working in projects makes your coding life much easier. R knows where to look for things, R knows where to output things, everything is organised, you will thank yourself later.
...
Note: When you open up the project in the future, just click on the .Rproj file, this will make sure your are actually working in that project. You can see which project you're working in at the top right of your Rstudio. See @fig-rproj.
...
When you write in scripts, you can "send" that code to the console to run, and you can save the script and come back to it later. Not only is that important for you personally working on big R projects, but its important when you come to publishing your code when you publish a paper.
...
R likes **comma separated value files (.csv)**, it does not like .xlsx files as in the back end of the file there's heaps of extra information that Excel uses and that R does not care about (e.g. colours of excel cells etc.). R just wants the raw data, nothing else.

##### Coding

The first thing is normally do with a new script, is to load the `tidyverse` package. This package is actually a collection of packages, and is just a lot of code that has been developed to make coding in R much more friendly. It is the new approach to coding in R and its much better to learn 'the tidyverse way'. People talk about "Base R" vs "the Tidyvese", you will ultimately learn both. But start with the tidyverse.
...
One way to overcome this is the take an average across the sites.
...
lm1 <- lm(count ~ year, data = dat_one_spp_mean) 
...
lmer1 <- lmerTest::lmer(count ~ year + (1|site_code), data = dat_one_spp) 
summary(lmer1)

```

By including all the data, we have significant decline in the count of the species.
...
dat_one_spp |> 
  mutate(prediction = predict(lmer1)) |> 
  ggplot(aes(x = year, 
             y = log(count))) + 
